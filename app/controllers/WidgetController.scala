...
class WidgetController @Inject()(cc: MessagesControllerComponents) extends MessagesAbstractController(cc) {
  ...
  def index = Action {
    Ok(views.html.indexs())
  }

  def insertData(): Action[AnyContent] = Action { implicit request =>
    database.withConnection { conn =>
  val statement = conn.prepareStatement(
    """
      |INSERT INTO public."Widgets" (name, price)
      |VALUES (?, ?)
    """.stripMargin
  )
  statement.setString(1, "appleWatch")
  statement.setDouble(2, 12000)
  statement.executeUpdate()
  statement.close()
  Ok(s"Inserted rows into the database.")
    }
  }
